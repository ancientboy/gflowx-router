...
这会在 `backend/` 内生成 `gflowxscene/` 并改写 `controller/relay.go`。若你已将 `backend` 推送到 **自有的 new-api fork**，请把 `.gitmodules` 的 `url` 改为 fork 地址，并把补丁后的提交推到该 fork，团队成员即可直接依赖子模块指针而无需每次 `git am`。
